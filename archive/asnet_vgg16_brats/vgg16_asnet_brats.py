...
# Using float32 as requested, change if using mixed precision
...
def AS_Net(input_size=(IMG_HEIGHT, IMG_WIDTH, INPUT_CHANNELS)):
    ...
    inputs = Input(
        input_size, dtype=tf.float32)  # Ensure input dtype is float32
    ...
    VGGnet = VGG16(weights="imagenet", include_top=False,
                   input_tensor=inputs)  # Pass input tensor directly
    ...
    # Adjust index based on model.summary() if needed
    for layer in VGGnet.layers[11:]:
    ...
    # Input shapes: (H, W, 32) + (H, W, 32) -> Synergy Conv(filters=1) -> (H, W, 1)
...
class SAM(Model):
    ...
    def __init__(self, filters, name='sam', **kwargs):
        ...
        self.conv4 = Conv2D(self.out_channels, 1, activation="relu", kernel_initializer="he_normal",
                            dtype=compute_dtype, name='conv4')  # Branch for attention weights (shortcut path)

        # Pooling and Upsampling
        self.pool1 = MaxPooling2D((2, 2), name='pool1')
        self.upsample1 = UpSampling2D(
            (2, 2), interpolation="bilinear", name='upsample1')
        self.W1 = Conv2D(self.out_channels, 1, activation="sigmoid",
                         kernel_initializer="he_normal", dtype=compute_dtype, name='W1')

        self.pool2 = MaxPooling2D((4, 4), name='pool2')
        self.upsample2 = UpSampling2D(
            (4, 4), interpolation="bilinear", name='upsample2')
        self.W2 = Conv2D(self.out_channels, 1, activation="sigmoid",
                         kernel_initializer="he_normal", dtype=compute_dtype, name='W2')
    ...
    def call(self, inputs):
        ...
        # Apply activation *before* W1
        activated1 = self.activation(upsampled1)
        merge1 = self.W1(activated1)  # S1

        pooled2 = self.pool2(out2)
        upsampled2 = self.upsample2(pooled2)
        # Apply activation *before* W2
        activated2 = self.activation(upsampled2)
...
class CAM(Model):
    ...
    def __init__(self, filters, reduction_ratio=16, name='cam', **kwargs):
        ...
        self.fc1 = Dense(self.out_channels // self.reduction_ratio,
                         activation="relu", use_bias=False, dtype=compute_dtype, name='fc1')
    ...
    def call(self, inputs):
        ...
        # Reshape to (Batch, 1, 1, C_out) for broadcasting
        out3 = self.reshape(fc2_out)

        # Apply attention: Multiply main path by channel attention weights and add shortcut
        # Y = F'(X) * C + F''(X)
        # Broadcasting applies channel weights F'(X) * C
        y = self.multiply([out1, out3])
        y = self.add([y, out2])  # Add shortcut connection F''(X)
        return y
...
class DiceLoss(Loss):
    ...
    def __init__(self, smooth=1e-6, name='dice_loss', **kwargs):
        ...
        super(DiceLoss, self).__init__(
            name=name, reduction='sum_over_batch_size', **kwargs)  # NEW
        self.smooth = smooth
...
class CombinedFocalDiceLoss(Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        combined = (self.focal_weight * focal_loss_val) + \
...
class CombinedLoss(Loss):
    ...
    def __init__(self, bce_weight=0.5, dice_weight=0.5, class_weight=1.0, name='combined_loss', **kwargs):
        ...
        self.bce_weight = bce_weight
        self.dice_weight = dice_weight
        # Instantiate WBCE with class weight
        self.wbce = WBCE(weight=class_weight)
...
def prepare_brats_data_gpu(
    ...
    # Filter out rows where the H5 file doesn't exist
            ...
            try:
                with h5py.File(path, "r") as hf:
                    ...
                    selected_modalities = image_data[:, :, modality_indices].astype(
                        np.float32)  # Shape (H, W, num_modalities_loaded)
                ...
                return (
                    np.zeros((original_h, original_w,
                             num_modalities_loaded), dtype=np.float32),
    ...
    def preprocess(image, mask):
        ...
        # Shape: (H, W, num_modalities_loaded)
...
def visualize_dataset_samples(dataset, num_samples=3, output_dir=OUTPUT_DIR):
    ...
    try:
        ...
        for images, masks in dataset.take(1):  # Take one batch
            ...
            print(
                f"Image value range: Min={tf.reduce_min(images):.4f}, Max={tf.reduce_max(images):.4f}")
            print(
            ...
            for i in range(min(num_samples, num_in_batch)):
                ...
                titles = ["Input R (T1ce)", "Input G (FLAIR)",
                          "Input B (T1ce)", "RGB Input", "Ground Truth Mask"]
                channels_to_plot = [img[:, :, 0],
                                    img[:, :, 1], img[:, :, 2], img, mask]
                ...
                for j, item in enumerate(channels_to_plot):
                    ...
                    if j < 3:  # Individual channels
                        ...
                        item_disp = np.clip(
                            (item - p_low) / (p_high - p_low + 1e-8), 0, 1) if (p_high - p_low) > 1e-7 else item
                        ...
                        img_rgb_disp = np.clip(item, 0, 1)
                        # Check for NaN/Inf
                        if np.isnan(img_rgb_disp).any() or np.isinf(img_rgb_disp).any():
                            print(
                                f"Warning: NaN/Inf found in RGB image display for sample {i}")
                            img_rgb_disp = np.nan_to_num(
                                img_rgb_disp)  # Replace NaN/Inf with 0
...
def train_model_distributed(
    ...
    if latest_checkpoint:
        ...
        try:
            # Use expect_partial for optimizer state flexibility
            ...
            try:
                ...
                epoch_str = filename.split(
                    '-')[0].split('.')[-1]  # Try common patterns
    ...
    callbacks = [
        ...
        tf.keras.callbacks.EarlyStopping(
            ...
            # Restore weights from best epoch (based on val_loss)
    ...
    # 4. Train the Model
    print(
        f"Starting training loop for {num_epochs - initial_epoch} potential epochs (may stop early)...")
    history = model.fit(
        train_dataset,
    ...
    # Return the final model state (might be from early stopping)
    ...
    try:
        ...
        for key in history_dict.keys():
            if key.startswith('val_'):
                continue  # Skip val metrics here
            if key == 'loss':
                continue
            if key == 'dice_coef':
        ...
        for metric, title in metrics_to_plot.items():
            ...
            if metric in history_dict:
                ...
                print(
                    f"Warning: Training metric '{metric}' not found in history.")
            ...
            if val_metric in history_dict:
                ...
                print(
                    f"Warning: Validation metric '{val_metric}' not found in history.")
    ...
    try:
        ...
        if model is None:
            ...
            if not os.path.exists(checkpoint_best_path):
                print(
                    f"Error: Best checkpoint file not found at {checkpoint_best_path}.")
                # Try loading the last epoch checkpoint instead
                last_checkpoint = tf.train.latest_checkpoint(
                    os.path.dirname(checkpoint_path))
                if not last_checkpoint:
                    print(
                        f"Error: No checkpoint found in {os.path.dirname(checkpoint_path)}. Cannot evaluate.")
                    return None  # Cannot proceed without a model
                else:
                    print(
                        f"Warning: Best checkpoint not found. Loading last epoch checkpoint: {last_checkpoint}")
            ...
            with strategy.scope():  # Load model within strategy scope if needed
                ...
                model_eval.load_weights(
                    checkpoint_to_load).expect_partial()  # Use expect_partial
                print(
            ...
            if not model.optimizer:
                ...
                with strategy.scope():
                    ...
                    # Assuming WBCE+Dice if not focal
        ...
        try:
            ...
            with open(perf_file_path, "w") as file_perf:
                ...
                file_perf.write("-------------------\n")
                file_perf.write(
                    f"Loss: {evaluation_results.get('loss', 'N/A'):.4f}\n")
                file_perf.write(
                    f"Binary Accuracy: {evaluation_results.get('binary_accuracy', 'N/A'):.4f}\n")
                file_perf.write(
                    f"Dice Coefficient: {evaluation_results.get('dice_coef', 'N/A'):.4f}\n")
                file_perf.write(
                    f"IoU (Jaccard): {evaluation_results.get('iou', 'N/A'):.4f}\n")
                file_perf.write(
                    f"Precision (Threshold={threshold}): {evaluation_results.get('precision', 'N/A'):.4f}\n")
                file_perf.write(
                    f"Recall/Sensitivity (Threshold={threshold}): {evaluation_results.get('recall', 'N/A'):.4f}\n")
                file_perf.write(
                    f"F1-Score (Threshold={threshold}): {evaluation_results.get('f1_score', 'N/A'):.4f}\n")
    ...
    try:
        ...
        for images, masks in dataset.take(1):
            ...
            for j in range(min(num_examples, images.shape[0])):
                ...
                plt.imshow(gt_mask_display, cmap='viridis', alpha=0.5,
                           vmin=0, vmax=1)  # Use squeeze for mask (H,W)
                plt.axis("off")
...
def create_completion_notification(output_folder=OUTPUT_DIR, completion_file=COMPLETION_FILE, start_time=None):
    ...
    try:
        with open(completion_file, "w") as f:
            ...
            f.write("- Model: AS-Net with VGG16 encoder\n")
...
evaluation_results = evaluate_model(
    model=model_to_evaluate)  # Pass model if it exists
