...
# 0 = all, 1 = INFO filtered, 2 = WARNING filtered, 3 = ERROR filtered
...
COMBINED_LOSS_WEIGHTS = {'bce_weight': 0.5, 'dice_weight': 0.5,
                         'class_weight': 100.0}  # Tunable loss weights
...
# Make sure this points to the directory containing 'BraTS20 Training Metadata.csv' and 'content/data'
DATASET_PATH = "brats2020-training-data/"
METADATA_FILE = os.path.join(DATASET_PATH, "BraTS20 Training Metadata.csv")
# Directory containing the .h5 slice files
...
if gpus:
    try:
        ...
        # Fallback to default (CPU or single GPU)
...
print("Global Batch Size (per replica * num replicas):",
...
def AS_Net(input_size=(IMG_HEIGHT, IMG_WIDTH, INPUT_CHANNELS), variant=MOBILENET_VARIANT):
    ...
    if variant == 'Large':
        ...
        base_model = MobileNetV3Large(
            ...
            # Pass preprocessed_inputs directly to the base model
        ...
        skip_layer_names = [
            ...
            # ~H/16 (14x14) - Changed from expanded_conv_8_add
            'expanded_conv_7_add',
        ...
        raise ValueError(
            "Invalid MOBILENET_VARIANT. Choose 'Large' or 'Small'.")
    ...
    for name in skip_layer_names:
        try:
            ...
            print(
                f"Available layers containing '{name.split('_')[0]}': {[l.name for l in base_model.layers if name.split('_')[0] in l.name]}")
            raise
    ...
    if variant == 'Large':
        ...
        print(
            f"Feature maps - H/2:{output1.shape}, H/4:{output2.shape}, H/8:{output3.shape}, H/16:{output4.shape}, H/32:{bottleneck.shape}")

        # Check if shapes match our expectations
        # If output4 is actually 7x7 (H/32) instead of 14x14 (H/16)
        if output4.shape[1] == 7:
            ...
            h32_features = concatenate(
                [output4, bottleneck], axis=-1, name='h32_combine')
            print(f"Combined H/32 features shape: {h32_features.shape}")

            # Upscale to H/16
            up16 = UpSampling2D((2, 2), interpolation="bilinear",
                                name='up_to_h16')(h32_features)
    ...
    # Small variant handling (similar pattern)
        ...
        if output4.shape[1] == 7:
            ...
            # Original flow if feature maps match expectations
            print(
                "Feature map shapes for Small variant - initializing standard decoder architecture.")
            ...
            # Decoder Stage 1: H/32 -> H/16
            up5 = UpSampling2D(
                (2, 2), interpolation="bilinear", name='up5')(bottleneck)
            # Combine with both H/16 feature maps since we have two at this level
            merge4 = concatenate([output3, output4, up5],
                                 axis=-1, name='merge4')
            filters4 = merge4.shape[-1]
            print(f"Decoder stage 1 (H/16): Input filters={filters4}")
            SAM4 = SAM(filters=filters4, name='sam4')(merge4)
            ...
            final_up_sam = UpSampling2D(
                (2, 2), interpolation="bilinear", name='final_up_sam')(up_sam1)
            final_up_cam = UpSampling2D(
                (2, 2), interpolation="bilinear", name='final_up_cam')(up_cam1)
...
class SAM(Model):
    ...
    def __init__(self, filters, name='sam', **kwargs):
        ...
        # Ensure at least 16 channels
        self.out_channels = max(16, filters // 4)
        ...
        self.conv3 = Conv2D(self.out_channels, 3, activation="relu", padding="same",
                            kernel_initializer="he_normal", dtype=compute_dtype, name='conv3')  # F'(X) path
        self.conv4 = Conv2D(self.out_channels, 1, activation="relu", kernel_initializer="he_normal",
                            dtype=compute_dtype, name='conv4')  # F''(X) path (shortcut)
        ...
        self.W1 = Conv2D(1, 1, activation="sigmoid", kernel_initializer="he_normal",
                         dtype=compute_dtype, name='W1')  # Output 1 channel for attention map
        ...
        self.W2 = Conv2D(1, 1, activation="sigmoid", kernel_initializer="he_normal",
                         dtype=compute_dtype, name='W2')  # Output 1 channel for attention map
    ...
    def call(self, inputs):
        ...
        # Element-wise addition S = S1 + S2 (shape: B, H, W, 1)
        attention_map = self.add_attention([att1, att2])
...
class CAM(Model):
    ...
    def __init__(self, filters, reduction_ratio=16, name='cam', **kwargs):
        ...
        # Keep dims for easier broadcasting later
    ...
    def call(self, inputs):
        ...
        # Global Average Pooling -> (Batch, 1, 1, C_out)
        pooled = self.gpool(out2)
        fc1_out = self.fc1(pooled)
        channel_attention_weights = self.fc2(
            fc1_out)  # Shape: (Batch, 1, 1, C_out)
        ...
        recalibrated_features = self.multiply(
...
class Synergy(Model):
    ...
    def __init__(self, alpha_init=0.5, beta_init=0.5, name='synergy', **kwargs):
        ...
        compute_dtype = mixed_precision.global_policy().compute_dtype
        self.conv = Conv2D(1, 1, padding="same", kernel_initializer="he_normal",
                           dtype=compute_dtype, name='conv')  # Output 1 channel
        self.bn = BatchNormalization(name='bn')
        self.add = Add(name='add_weighted')  # Use Add layer

    def call(self, inputs):
        # Expecting a tuple/list of (SAM_output, CAM_output)
...
class DiceCoefficient(Metric):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        # Use float32 for internal calculations
...
def prepare_brats_data_gpu(
    ...
    train_dataset = (
        ...
        # Shuffle file paths BEFORE loading for better randomness
        .shuffle(buffer_size)
...
def visualize_dataset_samples(dataset, num_samples=3, output_dir=OUTPUT_DIR):
    ...
    try:
        ...
        for images, masks in dataset.take(1):  # Take one batch
            ...
            print(
                f"Batch shapes - Images: {images.shape}, Masks: {masks.shape}")
            ...
            print(
                f"Image value range (Z-scored): Min={tf.reduce_min(images):.4f}, Max={tf.reduce_max(images):.4f}")
            print(
                f"Mask value range: Min={tf.reduce_min(masks):.4f}, Max={tf.reduce_max(masks):.4f}")
            print(
                f"Unique mask values in batch: {tf.unique(tf.reshape(masks, [-1]))[0]}")
            ...
            for i in range(min(num_samples, num_in_batch)):
                ...
                titles = ["Input R (T1ce Z)", "Input G (FLAIR Z)", "Input B (T1ce Z)",
                          "RGB Input (Z-scored)", "Ground Truth Mask"]
                channels_to_plot = [
                    img_zscored[:, :, 0], img_zscored[:, :, 1], img_zscored[:, :, 2], img_zscored, mask]

                for j, item in enumerate(channels_to_plot):
                    ax = plt.subplot(num_samples, len(titles),
                                     i * len(titles) + j + 1)
                    ...
                    if j < 4:  # Image data (Z-scored)
                        ...
                        p_low, p_high = np.percentile(
                            item.flatten(), [2, 98])  # Use flatten for RGB
                        item_disp = np.clip(
                            (item - p_low) / (p_high - p_low + 1e-8), 0, 1)
                        if np.isnan(item_disp).any() or np.isinf(item_disp).any():
                            print(
                                f"Warning: NaN/Inf found in image display for sample {i}, plot {j}")
                            item_disp = np.nan_to_num(
                                item_disp)  # Replace NaN/Inf
                        # Use cmap only for single channel
                        plt.imshow(
                            item_disp, cmap='gray' if item.ndim == 2 else None)
                    else:  # Mask
                        if np.isnan(item).any() or np.isinf(item).any():
                            print(
                                f"Warning: NaN/Inf found in mask display for sample {i}")
                            item = np.nan_to_num(item)
                        # Use 'jet' or 'viridis' for mask
...
def train_model_distributed(
    ...
    metrics=['binary_accuracy',
             ...
             tf.keras.metrics.Precision(
                 thresholds=THRESHOLD, name="precision"),
    ...
    # Include variant in log
    print(f"--- Starting Training ({VARIANT_SUFFIX}) ---")
    ...
    visualize_dataset_samples(
        train_dataset, num_samples=3, output_dir=output_dir)

    # 2. Build and Compile Model within Strategy Scope
    with strategy.scope():
        print("Building model...")
        # Pass variant to the model function
        model = model_func(input_size=(img_height, img_width,
                           input_channels), variant=mobilenet_variant)
        print("Model built.")
    ...
    if latest_checkpoint:
        ...
        try:
            ...
            try:
                ...
                numbers = [int(s) for s in filename.replace(
                    '.weights.h5', '').split('_') if s.isdigit()]
                if not numbers:  # Try splitting by '.' or '-'
                    parts = filename.replace(
                        '.weights.h5', '').replace('-', '.').split('.')
                    numbers = [int(s) for s in parts if s.isdigit()]

                if numbers:
                    # Take the largest number as likely epoch
                    initial_epoch = max(numbers)
                    print(
                ...
                print(
                    f"Warning: Could not determine epoch number from checkpoint name '{latest_checkpoint}', starting from epoch 0. Error: {parse_err}")
                initial_epoch = 0
        except Exception as load_err:
            print(
                f"Error loading weights from {latest_checkpoint}: {load_err}. Starting training from scratch.")
            initial_epoch = 0
    else:
    ...
    callbacks = [
        ...
        tf.keras.callbacks.ModelCheckpoint(
            # Use variant path (overwrites each epoch)
    ...
    # 4. Train the Model
    print(
        f"Starting training loop for max {num_epochs} epochs (current: {initial_epoch})...")
    history = model.fit(
        train_dataset,
...
def plot_training_history(history, output_dir=OUTPUT_DIR):
    ...
    try:
        ...
        for key in history_dict.keys():
            ...
            # Add known metrics
            if key == 'dice_coef':
                metrics_to_plot['dice_coef'] = 'Dice Coefficient'
            elif key == 'iou':
                metrics_to_plot['iou'] = 'IoU (Jaccard)'
            elif key == 'binary_accuracy':
                metrics_to_plot['binary_accuracy'] = 'Binary Accuracy'
            elif key == 'precision':
                metrics_to_plot['precision'] = 'Precision'
            elif key == 'recall':
                metrics_to_plot['recall'] = 'Recall'
        ...
        if num_plots <= 1:  # Only loss found
            ...
            if num_plots == 0:
                return  # Should not happen if loss exists
        ...
        for metric, title in metrics_to_plot.items():
            ...
            if val_metric in history_dict:
                ...
                if metric in history_dict:
                    plt.legend()  # Show legend only for training if val is missing
            ...
            if metric != 'loss' and val_metric in history_dict:  # Only add legend if both train/val exist
                plt.legend()
            if metric != 'loss':
                plt.ylim([0, 1.05])  # Limit y-axis for common metrics [0, 1]
            plt.grid(True)
            plot_index += 1

        # Add main title
        plt.suptitle(
            f'AS-Net {MOBILENET_VARIANT} Training History', fontsize=14)
...
def evaluate_model(
    ...
    try:
        ...
        print(
            f"Loading validation data with batch size: {global_eval_batch_size}...")
        _, val_dataset = dataset_func(
            batch_size=global_eval_batch_size, validation_split=0.2)  # Ensure consistent split
        ...
        if model is None:
            print(
                f"Loading model weights from best checkpoint: {checkpoint_best_path}")
            checkpoint_to_load = None
            if os.path.exists(checkpoint_best_path):
                checkpoint_to_load = checkpoint_best_path
            else:
                print(
                    f"Warning: Best checkpoint not found at {checkpoint_best_path}.")
            ...
            with strategy.scope():  # Load model within strategy scope
                # Pass variant to the model function
                model_eval = AS_Net(input_size=(
                    img_height, img_width, input_channels), variant=mobilenet_variant)
                ...
                # Optimizer state isn't used for eval but needed for compile
                ...
                model_eval.compile(
                    ...
                    loss=loss_instance,
                    metrics=['binary_accuracy',
                             # Ensure metrics are instantiated
                             DiceCoefficient(name='dice_coef'),
                             IoU(name='iou'),
                             tf.keras.metrics.Precision(
                                 thresholds=threshold, name="precision"),
                ...
                # Allow loading even if optimizer state is missing/different
                load_status.expect_partial()
                # load_status.assert_consumed() # Uncomment to check all weights were loaded
                print(
                    f"Successfully loaded weights into new model instance from {checkpoint_to_load}")
        else:
        ...
        # Add F1 to the results dictionary
        ...
        if 'val_dataset' in locals():
            del val_dataset  # Delete dataset
        if 'model_eval' in locals() and model_eval is not model:
            del model_eval  # Delete model if loaded here
        gc.collect()
...
def save_prediction_examples(model, dataset, output_folder, num_examples=5, threshold=THRESHOLD):
    ...
    try:
        ...
        for images, masks in dataset.take(1):
            ...
            # Images are already preprocessed by the model
            ...
            for j in range(min(num_examples, images_numpy.shape[0])):
                ...
                if np.isnan(img_display).any() or np.isinf(img_display).any():
                    print(
                        f"Warning: NaN/Inf found in input image display for example {j}")
                ...
                if np.isnan(gt_mask_display).any() or np.isinf(gt_mask_display).any():
                    print(
                        f"Warning: NaN/Inf found in GT mask display for example {j}")
                ...
                if np.isnan(pred_prob_display).any() or np.isinf(pred_prob_display).any():
                    print(
                        f"Warning: NaN/Inf found in probability map display for example {j}")
                ...
                if np.isnan(binary_pred_display).any() or np.isinf(binary_pred_display).any():
                    print(
                        f"Warning: NaN/Inf found in binary prediction display for example {j}")
...
def create_completion_notification(output_folder=OUTPUT_DIR, completion_file=COMPLETION_FILE, start_time=None):
    ...
    try:
        with open(completion_file, "w") as f:
            f.write(
                f"AS-Net ({VARIANT_SUFFIX}) Training Completed at: {timestamp}\n\n")
            f.write("Training Configuration:\n")
            f.write(
                f"- Model: AS-Net with MobileNetV3-{MOBILENET_VARIANT} encoder\n")
            ...
            f.write(
                f"- Global Batch size: {BATCH_SIZE * strategy.num_replicas_in_sync}\n")
            ...
            if os.path.exists(perf_file_path):
                try:
                    ...
                    f.write(
                        f"Note: Error reading performance metrics file ({perf_file_path}): {read_err}\n")
            else:
                f.write(
                    f"Note: Performance metrics file not found ({perf_file_path}). Evaluation might have failed or not run yet.\n")
...
if os.path.exists(COMPLETION_FILE):
    ...
    print(
        f"Completion file '{COMPLETION_FILE}' not found. Starting training process...")
    # Pass MOBILENET_VARIANT to the training function
    history, model = train_model_distributed(
        mobilenet_variant=MOBILENET_VARIANT)  # history and model are returned
...
evaluation_results = evaluate_model(
    model=model, mobilenet_variant=MOBILENET_VARIANT)  # Pass model if it exists
