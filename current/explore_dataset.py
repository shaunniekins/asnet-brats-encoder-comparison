...
def visualize_healthy_brain_tissue(file_path, output_dir):
    """Visualize the healthy brain tissue (ground truth) by showing regions not affected by tumor."""
    try:
        image_data, mask_data = load_h5_slice(file_path)
        modality_names = get_modality_names()

        # Create a binary mask for all tumor regions combined
        tumor_mask = np.zeros(mask_data.shape[:2], dtype=bool)
        for i in range(mask_data.shape[2]):
            tumor_mask |= mask_data[:, :, i] > 0

        # Create a brain mask (assumption: non-zero pixels in T1ce represent brain tissue)
        # T1ce typically provides good contrast for brain structures
        brain_mask = image_data[:, :, 1] > 0  # Using T1ce (index 1)

        # Healthy brain tissue = brain tissue minus tumor regions
        healthy_tissue_mask = brain_mask & ~tumor_mask

        # Create visualization
        fig, axes = plt.subplots(1, 3, figsize=(15, 5))

        # Original T1ce image
        axes[0].imshow(image_data[:, :, 1], cmap='gray')
        axes[0].set_title('Original T1ce Image')
        axes[0].axis('off')

        # Tumor regions
        tumor_overlay = np.zeros_like(image_data[:, :, 1])
        tumor_overlay[tumor_mask] = 1
        axes[1].imshow(image_data[:, :, 1], cmap='gray')
        axes[1].imshow(tumor_overlay, cmap='Reds', alpha=0.5)
        axes[1].set_title('Tumor Regions')
        axes[1].axis('off')

        # Healthy brain tissue (ground truth)
        healthy_overlay = np.zeros_like(image_data[:, :, 1])
        healthy_overlay[healthy_tissue_mask] = 1
        axes[2].imshow(image_data[:, :, 1], cmap='gray')
        axes[2].imshow(healthy_overlay, cmap='Greens', alpha=0.5)
        axes[2].set_title('Healthy Brain Tissue (Ground Truth)')
        axes[2].axis('off')

        plt.suptitle(
            f"Healthy Brain Tissue vs Tumor: {os.path.basename(file_path)}")
        plt.tight_layout(rect=[0, 0.03, 1, 0.95])

        save_filename = f"healthy_brain_{os.path.basename(file_path).replace('.h5', '.png')}"
        save_path = os.path.join(output_dir, save_filename)
        plt.savefig(save_path, dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f"  Saved healthy brain visualization to: {save_path}")

    except Exception as e:
        print(
            f"Error visualizing healthy brain tissue for {os.path.basename(file_path)}: {e}")
        traceback.print_exc()
...
if __name__ == "__main__":
    ...
    if metadata_df is not None and not metadata_df.empty and h5_files_list:
        ...
        # Add visualization of healthy brain tissue
        visualize_healthy_brain_tissue(sample_file_path, EXPLORE_OUTPUT_DIR)
