...
# Parse command-line arguments to determine the variant
...
if len(sys.argv) > 1:
    ...
    # Default to B0 if no argument is provided
    EFFICIENTNET_VARIANT = 'EfficientNetV2B0'
...
# Update completion file location to use a dedicated folder
COMPLETION_FOLDER = "completion-notifications"  # New dedicated folder
COMPLETION_FILE = f"{COMPLETION_FOLDER}/{VARIANT_SUFFIX}-asnet-finished-training.txt"
...
# Create completion notifications folder
os.makedirs(COMPLETION_FOLDER, exist_ok=True)

# --- New Constant ---
# 0:T1, 1:T1ce, 2:T2, 3:FLAIR (Defaulting to FLAIR)
REFERENCE_MODALITY_INDEX_FOR_DISPLAY = 3
...
def AS_Net_EfficientNetV2(input_size=(IMG_HEIGHT, IMG_WIDTH, INPUT_CHANNELS), variant='EfficientNetV2B0'):
    ...
    # FIX: Use a custom Keras Layer instead of direct tf.image.resize
    # Create a fixed size target to resize to using ResizeToMatchLayer
    input_shape_tensor = Input(
        (target_h, target_w, 1), name='target_shape_input')

    # Use ResizeToMatchLayer to resize final outputs to match input size
    final_up_sam_resized = ResizeToMatchLayer(
        name='final_resize_sam')([final_up_sam, input_shape_tensor])
    final_up_cam_resized = ResizeToMatchLayer(
        name='final_resize_cam')([final_up_cam, input_shape_tensor])
    ...
    # Create model with the primary input and dummy input for shape reference
    model = Model(inputs=[inputs, input_shape_tensor],
                  outputs=output, name=f'AS_Net_{variant}')

    # Create a wrapper model that automatically handles the dummy input
    def get_final_model():
        inp = Input(input_size, dtype=tf.float32, name='image_input')
        # Create a dummy tensor with the target shape
        dummy = tf.ones((1, target_h, target_w, 1))
        out = model([inp, dummy])
        return Model(inputs=inp, outputs=out, name=f'AS_Net_{variant}_Wrapper')

    # Return the wrapped model (which only needs a single input)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(COMPLETION_FILE):
        ...
        history, model_instance = train_model_distributed(
            ...
            # Pass new constant
            reference_modality_index=REFERENCE_MODALITY_INDEX_FOR_DISPLAY
        )
    else:
