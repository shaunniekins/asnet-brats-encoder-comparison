...
# 3. Building Model using AS_Net with MobileNetV3, VGG16, and EfficientNetV2L
...
# Instantiate models for each encoder type
encoders = ['mobilenetv3', 'vgg16', 'efficientnetv2']
models = {}

for encoder in encoders:
    print(f"Training with {encoder} encoder")
    models[encoder] = AS_Net(encoder=encoder)

    # Compile the model
    models[encoder].compile(Adamax(learning_rate=0.001),
                            loss='categorical_crossentropy',
                            metrics=['accuracy', Precision(), Recall()])

    # Model Summary
    models[encoder].summary()
    tf.keras.utils.plot_model(models[encoder], show_shapes=True)

    # 4. Training the model
    print(f"Training model with {encoder} encoder...")
    hist = models[encoder].fit(tr_gen,
                               epochs=10,
    ...
    print(f"Evaluating model with {encoder} encoder...")
    train_score = models[encoder].evaluate(tr_gen, verbose=1)
    valid_score = models[encoder].evaluate(valid_gen, verbose=1)
    test_score = models[encoder].evaluate(ts_gen, verbose=1)
    ...
    preds = models[encoder].predict(ts_gen)
    ...
    plt.title(f'Confusion Matrix for {encoder} Encoder')
    plt.show()
    ...
    filename = f'classification_report_{models[encoder]}.txt'
    ...
    def predict(img_path):
        ...
        img = img / 255
        predictions = models[encoder].predict(img)
