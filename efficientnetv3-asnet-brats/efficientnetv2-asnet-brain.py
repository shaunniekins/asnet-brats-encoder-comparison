...
from keras.layers import (
    ...
    Layer,  # Import Layer for ResizeToMatchLayer
)
...
if EFFICIENTNET_VARIANT == 'EfficientNetV2B0':
    ...
    print(
        f"Warning: Using default size 224x224 for unlisted variant {EFFICIENTNET_VARIANT}")
    IMG_HEIGHT = 224
    IMG_WIDTH = 224
...
def AS_Net(input_size=(IMG_HEIGHT, IMG_WIDTH, INPUT_CHANNELS), variant=EFFICIENTNET_VARIANT):
    ...
    print(
        f"Fine-tuning: Freezing layers up to block '{freeze_until_block}'...")
    layer_found = False
    for layer in base_model.layers:
        if freeze_until_block in layer.name:
            layer_found = True
            print(
                f"Reached block '{freeze_until_block}' at layer: {layer.name}. Unfreezing subsequent layers.")
        if not layer_found:
    ...
    if not layer_found:
        print(
            f"Warning: Block '{freeze_until_block}' not found for freezing. Fine-tuning all layers.")
        ...
        try:
            ...
            for i, layer in enumerate(base_model.layers):
                ...
                if hasattr(layer, 'output_shape'):
                    print(
                        f"  {i}: {layer.name} - Output Shape: {layer.output_shape}")
                else:
                    print(f"  {i}: {layer.name}")
                if i > 50:
                    break  # Print only first few layers
            raise ValueError(
    ...
    print("Decoder Stage 2 (H/16 -> H/8)")
    ...
    # Input shapes: (H, W, C_out_synergy) -> Output: (H, W, 1)
...
class SAM(Model):
    ...
    def __init__(self, filters, name='sam', **kwargs):
        ...
        # Adjust reduction based on input filters
        ...
        self.W1 = Conv2D(1, 1, activation="sigmoid", kernel_initializer="he_normal",
                         dtype=compute_dtype, name='W1')  # Output 1 channel attention map
        ...
        self.W2 = Conv2D(1, 1, activation="sigmoid", kernel_initializer="he_normal",
                         dtype=compute_dtype, name='W2')  # Output 1 channel attention map
...
class CAM(Model):
    ...
    def __init__(self, filters, reduction_ratio=16, name='cam', **kwargs):
        ...
        self.gpool = GlobalAveragePooling2D(
            name='global_avg_pool', keepdims=True)  # Keep dims for broadcasting
...
class Synergy(Model):
    ...
    def call(self, inputs):
        # Expecting tuple/list: (SAM_output, CAM_output)
...
def prepare_brats_data_gpu(
    ...
    h5_dir=H5_DATA_DIR,
    # Use updated target size from constants
    ...
    try:
        ...
        for images, masks in dataset.take(1):  # Take one batch
            ...
            for i in range(min(num_samples, num_in_batch)):
                ...
                titles = ["Input R (Z)", "Input G (Z)", "Input B (Z)",
                ...
                for j, item in enumerate(channels_to_plot):
                    ...
                    if j < 4:  # Z-scored image data
                        ...
                        if np.isnan(item_disp).any() or np.isinf(item_disp).any():
                            item_disp = np.nan_to_num(item_disp)
                        plt.imshow(
...
def train_model_distributed(
    ...
    if latest_checkpoint:
        ...
        try:
            ...
            try:
                epoch_str = latest_checkpoint.split(
                    '_')[-1].split('.')[0]  # Try common patterns
                if not epoch_str.isdigit():
                    epoch_str = latest_checkpoint.split(
                        '.')[-2].split('-')[-1]  # Another pattern
    ...
    print(
        f"Starting training loop for {epochs_to_run} epochs (Total planned: {num_epochs})...")
    if epochs_to_run <= 0:
    ...
    try:
        ...
        for key in history_dict.keys():
            ...
            if key == 'loss':
                continue
            if key == 'dice_coef':
                metrics_to_plot['dice_coef'] = 'Dice Coef'
            elif key == 'iou':
                metrics_to_plot['iou'] = 'IoU'
            elif key == 'binary_accuracy':
                metrics_to_plot['binary_accuracy'] = 'Accuracy'
            elif key == 'precision':
        ...
        for metric, title in metrics_to_plot.items():
            ...
            plt.title(f'{title}')
            plt.xlabel('Epoch')
            if metric != 'loss' and val_metric in history_dict:
                plt.legend()
            elif metric in history_dict and val_metric not in history_dict:
                plt.legend()  # Show training legend if val missing
            if metric != 'loss':
                plt.ylim([0, 1.05])
            plt.grid(True)
            plot_index += 1
        ...
        plt.suptitle(
            f'AS-Net {EFFICIENTNET_VARIANT} Training History', fontsize=14)
...
def save_prediction_examples(model, dataset, output_folder, num_examples=5, threshold=THRESHOLD):
    ...
    try:
        for images, masks in dataset.take(1):  # Take one batch
            ...
            # Model handles its internal preprocessing
