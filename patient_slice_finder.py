...
def find_patient_slices(h5_test_dir, patient_id=None, list_patients=False, show_examples=False):
    ...
    if not os.path.isdir(h5_test_dir):
        raise FileNotFoundError(f"Test slices directory not found: {h5_test_dir}")
    ...
    if list_patients:
        patient_counts = [(p_id, len(slices)) for p_id, slices in patient_slices.items()]
    ...
    if patient_id:
        ...
        print(f"Found {len(patient_slice_files)} slices for patient {patient_id}:")
                ...
                if idx < len(patient_slice_files):
                    try:
                        with h5py.File(patient_slice_files[idx], 'r') as f:
                            ...
                            plt.title(f"Slice {idx+1}/{len(patient_slice_files)}")
                            plt.axis('off')
                    except Exception as e:
                        print(f"Error displaying slice {patient_slice_files[idx]}: {e}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Find slices for specific patients in BraTS test dataset")
    parser.add_argument("--test_dir", type=str, default="./preprocessed_brats23_h5_slices/test",
